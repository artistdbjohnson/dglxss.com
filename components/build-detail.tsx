...
const PHASES = {
  ...
  pt: [
    "Design & sourcing de peças",
    "Construir primeiras unidades",
    "Self-test no próprio camião",
    "Testar com amigos do trucking",
    "Testar com empresa conhecida",
  ],
} as const;

export function BuildDetail({ project }: { project: PortfolioProject }) {
    ...
    <div className="h-dvh max-h-dvh overflow-hidden bg-black text-white flex flex-col items-center justify-center px-4 sm:px-6">
      <div className="w-full max-w-lg lg:max-w-xl flex flex-col max-h-[min(92dvh,760px)]">
        <div className="flex items-center justify-between gap-3 mb-5 sm:mb-6">
          <Link
            href="/#in-progress"
            className="inline-flex items-center gap-2 text-sm text-white/55 hover:text-white transition-colors min-h-9 self-start"
          >
            <ArrowLeft size={15} />
            {locale === "pt" ? "Voltar ao estúdio" : "Back to studio"}
          </Link>
          <LanguageSwitch />
        </div>
